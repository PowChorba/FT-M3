...
async function problemA () {
  ...
  async function readFiles(files){
    blue(await promisifiedReadFile(files))
  }

  await Promise.all([readFiles('poem-two/stanza-01.txt'),
  readFiles('poem-two/stanza-02.txt')])
  console.log('done')
}
...
async function problemB () {
  ...
  const promesas = filenames.map(s => readFiles(s))
