...
function problemB () {
  ...
  // AsyncAwait version
  async function readFileName(filename){
    blue(await promisifiedReadFile(filename))
  }
  readFileName('poem-one/stanza-02.txt')
  readFileName('poem-one/stanza-03.txt')
}
...
async function problemC () {
  ...
  // AsyncAwait version
  const stanzaDos = await promisifiedReadFile('poem-one/stanza-02.txt')
  const stanzaTres = await promisifiedReadFile('poem-one/stanza-03.txt')
  blue(stanzaDos)
  blue(stanzaTres)
  console.log('done')
}
...
function problemD () {
  ...
  // AsyncAwait version
  async function cuatro(filename){
    try{
      blue(await promisifiedReadFile(filename))
    }
    catch(err){
  ...
  cuatro('poem-one/wrong-file-name.txt')
}
...
async function problemE () {
  ...
  try{
    const stanzatres = await promisifiedReadFile('poem-one/stanza-03.txt')
    const stanzacuatro = await promisifiedReadFile('poem-one/wrong-file-name.txt')
    blue(stanzatres)
    blue(stanzacuatro)
  }catch(err){
    magenta(err)
